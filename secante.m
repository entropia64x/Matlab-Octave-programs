## Copyright (C) 2020 Emilio Salcedo-Martinez

## -*- texinfo -*- 
## @deftypefn {} {@var{x} =} secante (@var{funcion}, @var{x0}, @var{x1})
##
## @seealso{}
## @end deftypefn

## Author: Emilio Salcedo-Martinez <salcedo.e@ugto.mx>
## Created: 2020-06-23

function [x,iter,err_abs] = secante (funcion,x0,x1,varargin)

  [f, funcion, quiereGrafica, intervalo, quiereTabla, idTabla, max_iter, tol] = inicializa (funcion,x0,x1,varargin,nargin);
  ...
  while ( iter <= max_iter )
    ...
    if ( quiereGrafica && ~iscomplex(x) )
      intervalo = graficaMetodo(f, x0, x1, x, intervalo, iter);
    end
    
    if ( quiereTabla )
      fprintf(idTabla,'%4d %0.8f %0.8f %0.8f %0.11f\n',iter,x0,x1,x,err_abs)
    end
...
function [f, funcion, quiereGrafica, intervalo, quiereTabla, idTabla, max_iter, tol] = inicializa (funcion,x0,x1,argentvar,nargent)
  
  if ( nargent < 3 || nargent > 9 )
    print_usage('secante');
  end
  ...
  if ( ~isnumeric(x0) || ~isscalar(x0) || ~isnumeric(x1) || ~isscalar(x1) )
  ...
  while ( ~isempty(argentvar) )
    ...
    elseif ( tolower(argentvar{1}) == 't' )
      ...
      fprintf(idTabla,'Metodo de la secante\n\n');
      fprintf(idTabla,'%4s %10s %10s %10s %13s\n','n','x_(n-2)','x_(n-1)','x_n','|x_n-x_(n-1)|');
      fprintf(idTabla,'---------------------------------------------------\n');
...
function intervalo = graficaMetodo(f, x0, x1, x, intervalo, iter)
  ...
  if ( iter == 1 )
    ...
    plot(x1,0,'s;x_1;','MarkerSize',8,'MarkerFaceColor','auto');
    plot([x0 x0], [0 f(x0)],'--','LineWidth',2);
    plot([x1 x1], [0 f(x1)],'--','LineWidth',2);
  end
  
  plot(x,0,'.','MarkerSize',20,'MarkerFaceColor','auto');
  plot([x x], [0 f(x)],'--','LineWidth',2);
  plot([x0 x1],[f(x0) f(x1)],':','LineWidth',2);
