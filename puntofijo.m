...
## -*- texinfo -*- 
## @deftypefn {} {@var{x} =} puntofijo (@var{f}, @var{x0})
## @deftypefnx {} {} puntofijo (@dots{}, @var{max_iter})
## @deftypefnx {} {} puntofijo (@dots{}, @var{max_iter}, @var{tol})
## @deftypefnx {} {} puntofijo (@dots{}, 'g', @var{intervalo})
## @deftypefnx {} {} puntofijo (@dots{}, 't')
## @deftypefnx {} {[@var{x}, @var{iter}] =} puntofijo (@dots{})
## @deftypefnx {} {[@var{x}, @var{iter}, @var{err_abs}] =} puntofijo (@dots{})
## 
...
##
## @example
## [x, iter, err_abs] = puntofijo (@@(x) x.^2 - 2, 1, 2)
## @end example
##
## @noindent
...
function [x, iter, err_abs] = puntofijo (funcion, x0, varargin)
  
  [g, funcion, intervalo, idTabla, max_iter, tol] = inicializacion (funcion, x0, varargin, nargin);
  ...
  while ( iter <= max_iter )
    ...
    if ( ~isbool(intervalo) && ~iscomplex(x) )
      intervalo = graficaMetodo(g, iter, x0, x, intervalo);
    end
  ...
  if ( ~isbool(intervalo) )
    graficaDetalles(intervalo,iter,x,g,funcion)
  end
...
function [g, funcion, intervalo, idTabla, max_iter, tol] = inicializacion (funcion, x0, argentvar, nargent)
  
  if ( nargent < 2 || nargent > 7 )
    print_usage('puntofijo');
  end
  ...
  while ( ~isempty(argentvar) )
    ...
    elseif ( tolower(argentvar{1}) == 't' )
      idTabla = fopen ('puntofijo.txt','w');
      fprintf (idTabla, 'Metodo de punto fijo\n\n');
